...
import { 
  Activity, HeartPulse, Users, 
  Stethoscope, Brain, ArrowUp, ChevronDown, BarChart2
} from 'lucide-react';
  ...
  return (
    <div className="absolute top-4 right-4 z-40">
      <button 
        ...
        className="flex items-center gap-1.5 text-xs font-bold bg-slate-800 hover:bg-slate-700 text-indigo-400 px-3 py-1.5 rounded-lg border border-slate-600 transition-colors shadow-lg"
      >
        <BarChart2 size={14} /> More about the chart
      </button>
      {isOpen && (
        <div className="absolute right-0 top-12 w-72 md:w-80 p-5 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl text-sm text-slate-200 animate-fadeIn text-left">
          <strong className="block text-indigo-400 mb-2 text-base">{title}</strong>
...
const TakeawayBanner = ({ text }) => (
  <div className="mt-4 p-5 bg-slate-800/60 border-l-4 border-amber-500 rounded-r-xl shadow-md text-slate-300 text-sm md:text-base leading-relaxed">
    <span className="font-black text-amber-500 uppercase text-xs tracking-wider block mb-1.5">Key Insight:</span>
    {text}
  </div>
);
...
export default function App() {
  ...
  const processedData = useMemo(() => {
    ...
    const calcRelDiff = (diaPct, hltPct) => hltPct > 0 ? ((diaPct - hltPct) / hltPct) * 100 : 0;
    ...
    const riskFactorTitles = {
      bp: 'High Blood Pressure', chol: 'High Cholesterol', smoker: 'Active Tobacco Smoker',
      heart: 'Heart Disease History', stroke: 'Prior Cerebrovascular Stroke',
      noPhysAct: 'Sedentary Lifestyle', diffWalk: 'Difficulty Walking'
    };
    ...
    // Calculate Butterfly chart structures with relative differences
    const buildButterfly = (labels, healthyCounts, diabeticCounts) => {
      const hPcts = healthyCounts.map(c => calcPct(c, stats.healthy.total));
      const dPcts = diabeticCounts.map(c => calcPct(c, stats.diabetic.total));
      const relDiffs = dPcts.map((d, i) => calcRelDiff(d, hPcts[i]));
      return {
        labels,
        healthy: hPcts.map(v => -v), // Negative for mirrored effect
        diabetic: dPcts,
        relDiffs 
      };
    };
    ...
    return {
      ...
      butterfly: {
        diet: buildButterfly(
          ['Physically Active', 'Eats Fruit Daily', 'Eats Vegetables Daily'],
          [stats.healthy.physAct, stats.healthy.fruits, stats.healthy.veggies],
          [stats.diabetic.physAct, stats.diabetic.fruits, stats.diabetic.veggies]
        ),
        barriers: buildButterfly(
          ['Difficulty Walking', 'Cannot Afford Doctor'],
          [stats.healthy.diffWalk, stats.healthy.noDoc],
          [stats.diabetic.diffWalk, stats.diabetic.noDoc]
        )
      },
      odds: oddsResult,
  ...
  const sankeyDetails = {
    0: { title: "Diabetes Diagnosis (Base Cohort)", text: "All 35,000+ diabetic records initiate here. In clinical practice, diabetes is a systemic metabolic disruption that damages fine structures of the vascular tree over time.", clinical: "Chronic hyperglycemia acts as a driver for endothelial dysfunction, creating baseline vulnerability to systemic cardiovascular decay." },
    1: { title: "High Blood Pressure Comorbidity", text: "Over 75% co-presents with chronic hypertension. This reveals how highly overlapping these physiological pathways are in real-world samples.", clinical: "Hypertension and diabetes are synergistic drivers for arterial stiffness and autonomic cardiovascular strain." },
    2: { title: "High Cholesterol Comorbidity", text: "Combined with high systemic glucose, floating lipids create accelerated atherosclerotic plaque deposits, restricting blood flow.", clinical: "Marked by small, highly dense LDL particles penetrating compromised vascular linings." },
    3: { title: "Heart Disease Intersection", text: "Chronic insulin resistance leads to ischemic tissue injuries and progressive coronary damage.", clinical: "Cardiovascular disease stands as the leading contributor to mortality among diabetic populations." },
    4: { title: "Stroke Comorbidity", text: "Diabetic individuals face multi-fold higher risk for cerebral blood vessel occlusion due to systemic vascular stiffening.", clinical: "Cerebrovascular risk tracks with prolonged mean arterial pressures and systemic stiffening." }
  };

  const activeSankeyStyles = useMemo(() => {
    switch (selectedNode) {
      case 0: return { border: "border-rose-500/40", bg: "bg-rose-950/25", badge: "bg-rose-500/20 text-rose-300" };
      case 1: return { border: "border-sky-500/40", bg: "bg-sky-950/25", badge: "bg-sky-500/20 text-sky-300" };
      case 2: return { border: "border-amber-500/40", bg: "bg-amber-950/25", badge: "bg-amber-500/20 text-amber-300" };
      case 3: return { border: "border-purple-500/40", bg: "bg-purple-950/25", badge: "bg-purple-500/20 text-purple-300" };
      case 4: return { border: "border-emerald-500/40", bg: "bg-emerald-950/25", badge: "bg-emerald-500/20 text-emerald-300" };
      default: return { border: "border-slate-700", bg: "bg-slate-900/50", badge: "bg-slate-800 text-slate-400" };
    }
  }, [selectedNode]);
  ...
  const cumulativePct = Math.min((cumulativeRisk / 50.0) * 100, 100);
  const allRiskKeys = Object.keys(calcRisks);

  // Split into left and right columns (4 and 3)
  const leftColKeys = allRiskKeys.slice(0, 4);
  const rightColKeys = allRiskKeys.slice(4, 7);

  const RiskBlock = ({ rKey }) => {
    const label = 
      rKey === 'bp' ? 'High Blood Pressure' : rKey === 'chol' ? 'High Cholesterol' : 
      rKey === 'smoker' ? 'Active Smoker' : rKey === 'heart' ? 'Heart Disease' :
      rKey === 'stroke' ? 'Stroke History' : rKey === 'noPhysAct' ? 'Sedentary Lifestyle' : 'Difficulty Walking';
    ...
    return (
      <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-3 shadow-inner h-full w-full flex flex-col justify-between">
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          />
          <span className="font-bold text-slate-200 text-sm hover:text-indigo-400 transition-colors select-none">
            {label}
          </span>
        </label>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className={`text-xs font-bold font-mono w-12 text-right transition-colors ${isActive ? 'text-indigo-400' : 'text-slate-600'}`}>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans overflow-y-auto scroll-smooth">
      <button onClick={scrollToTop} className="fixed bottom-6 right-6 z-50 p-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full shadow-2xl shadow-indigo-600/40 transition-transform transform hover:scale-110">
        <ArrowUp size={24} />
      </button>
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-4 md:gap-8 relative">
            <div className="group relative">
              <a href="#part1" className="flex items-center gap-1.5 text-sm font-semibold text-slate-300 hover:text-indigo-400 transition-colors py-2">Part 1: Patient Risk <ChevronDown size={14}/></a>
            ...
            <div className="group relative">
              <a href="#part2" className="flex items-center gap-1.5 text-sm font-semibold text-slate-300 hover:text-indigo-400 transition-colors py-2">Part 2: Socioeconomic <ChevronDown size={14}/></a>
              <div className="absolute hidden group-hover:block top-full left-0 mt-1 w-56 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl p-2 z-50">
            ...
            <div className="group relative">
              <a href="#part3" className="flex items-center gap-1.5 text-sm font-semibold text-slate-300 hover:text-indigo-400 transition-colors py-2">Part 3: Intersections <ChevronDown size={14}/></a>
      ...
      <header className="max-w-4xl mx-auto pt-14 pb-12 px-6 text-center">
        <h1 className="text-3xl md:text-5xl font-black text-white tracking-tight leading-tight">Mapping Diabetes Risk & Systemic Barriers</h1>
        <div className="mt-8 text-left text-sm max-w-2xl mx-auto">
          <div className="bg-indigo-950/25 p-5 rounded-2xl border border-indigo-900/35">
            ...
            <p className="text-indigo-200 leading-relaxed font-medium">This interactive dashboard translates complex numbers from 70,000 real people into simple wellness stories. Explore how our personal life situations (like household income and school access) combine with physical markers and common behaviors to shape our risk of developing diabetes.</p>
          </div>
          <div className="mt-6 flex justify-center">
            <button onClick={() => setDeepDiveIntro(!deepDiveIntro)} className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 rounded-lg hover:bg-indigo-500/20 hover:text-indigo-300 transition-all">
              <Brain size={16} /> {deepDiveIntro ? 'Hide Technical Details' : 'Professional Deep Dive'}
            </button>
          </div>
          {deepDiveIntro && (
            <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 mt-4 animate-fadeIn">
              ...
              <p className="text-slate-300 leading-relaxed">This dashboard analyzes 70,000 anonymized health records extracted from the CDC's Behavioral Risk Factor Surveillance System (BRFSS) 2015 dataset. Using calculated statistical parameters, multi-dimensional stratification, and correlation matrices, we explore non-linear logistic markers, demographic discrepancies, and cardiovascular overlaps.</p>
              <a href="https://www.kaggle.com/datasets/alexteboul/diabetes-health-indicators-dataset?resource=download" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 mt-4 text-xs text-indigo-400 hover:text-indigo-300 font-bold underline">
      ...
      <div className="max-w-5xl mx-auto px-6 pb-24 space-y-16">
        ...
        <section id="part1" className="space-y-8 scroll-mt-20">
          <div className="border-b border-slate-800 pb-4 mb-8">
            <h2 className="text-2xl font-bold text-white flex items-center gap-3"><HeartPulse className="text-rose-500" size={24} /> Analyzing Patient Risk: Clinical Drivers of Diabetes</h2>
          ...
          <div className="space-y-6">
            <div id="calculator" className="relative bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden p-6 space-y-6 scroll-mt-24">
              ...
              <div className="pr-12 lg:pr-32">
                ...
                <p className="text-sm text-slate-400 mt-1">Select physiological indicators to observe how metabolic risk parameters compound.</p>
              </div>
              ...
              <div className="p-5 bg-indigo-500/10 border border-indigo-500/20 rounded-xl space-y-3">
                ...
                <div className="bg-slate-950 h-4 rounded-full overflow-hidden border border-slate-800 p-0.5">
                  <div className="h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-rose-500 transition-all duration-500 ease-out rounded-full" style={{ width: `${cumulativePct}%` }} />
                </div>
              </div>
              
              <TakeawayBanner text="High blood pressure and high cholesterol create a multiplicative cardiovascular threat. An individual with both conditions is statistically over 10 times more likely to carry a diabetes diagnosis than someone with a clean baseline." />
            </div>

            <div id="barchart" className="relative bg-slate-900 border border-slate-800 rounded-2xl shadow-xl p-6 scroll-mt-24">
              <ChartInfoButton 
                title="Odds Ratios (OR)" 
                text="An odds ratio measures association strength. A value of 1.0 (the dotted red line) represents no difference in risk. Bars extending further to the right mathematically represent severely elevated prevalence." 
              />
              <div className="mb-4 pr-12 lg:pr-32">
              ...
              <div className="w-full h-[400px]">
                <Plot
                  data={[{ type: 'bar', orientation: 'h', x: processedData.sortedOddsX, y: processedData.sortedOddsY, marker: { color: '#4f46e5' }, text: processedData.sortedOddsX.map(val => val.toFixed(2) + 'x'), textposition: 'auto', hovertemplate: '<b>%{y}</b><br>Multiplies risk by %{x:.2f}x<extra></extra>' }]}
                  layout={{ font: { color: '#94a3b8' }, paper_bgcolor: 'transparent', plot_bgcolor: 'transparent', margin: { l: 200, r: 20, t: 20, b: 40 }, xaxis: { title: 'Risk Multiplier (Odds Ratio)', autorange: true }, yaxis: { autorange: true }, shapes: [{ type: 'line', x0: 1, x1: 1, y0: -0.5, y1: 7.5, line: { color: '#f43f5e', dash: 'dash', width: 2 } }], autosize: true }}
                  useResizeHandler={true} style={{ width: '100%', height: '100%' }}
                />
              </div>
              <TakeawayBanner text="Mobility limitations and high blood pressure independently stand out as the two largest absolute drivers of diabetes risk in this dataset." />
            </div>
          </div>
        ...
        <section id="part2" className="space-y-8 scroll-mt-20">
          <div className="border-b border-slate-800 pb-4 mt-16 mb-8">
            <h2 className="text-2xl font-bold text-white flex items-center gap-3"><Users className="text-amber-500" size={24} /> Social Stratification: Income, Education, and Access</h2>
          ...
          <div id="slopes" className="relative bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl w-full scroll-mt-24">
            ...
            />
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 pr-12 lg:pr-32">
              <div>
              ...
              <div className="flex bg-slate-950 p-1.5 rounded-xl border border-slate-800 shadow-inner">
                <button onClick={() => setSlopeView('income')} className={`px-4 py-2 text-sm rounded-lg font-bold transition-all ${slopeView === 'income' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>Income</button>
                <button onClick={() => setSlopeView('education')} className={`px-4 py-2 text-sm rounded-lg font-bold transition-all ${slopeView === 'education' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>Education</button>
              </div>
            </div>

            <div className="w-full h-[400px]">
              <Plot
                data={[{ type: 'scatter', mode: 'lines+markers', x: slopeView === 'income' ? incomeLabels : educationLabels, y: slopeView === 'income' ? processedData.slope.incomeY : processedData.slope.eduY, line: { color: slopeView === 'income' ? '#fbbf24' : '#22d3ee', width: 4 }, marker: { size: 10 }, hovertemplate: '<b>%{x}</b><br>Diabetes Rate: %{y:.1f}%<extra></extra>' }]}
                layout={{ font: { color: '#94a3b8' }, paper_bgcolor: 'transparent', plot_bgcolor: 'transparent', xaxis: { title: slopeView === 'income' ? 'Annual Household Income' : 'Education Level', autorange: true }, yaxis: { title: 'Prevalence of Diabetes (%)', autorange: true }, autosize: true }}
                useResizeHandler={true} style={{ width: '100%', height: '100%' }}
              />
            </div>
            <TakeawayBanner text={slopeView === 'income' ? "A steep gradient persists across household income classes: the poorest earners exhibit more than double the diabetes rate of those in upper-income classes." : "Educational achievement functions as a protective socioeconomic proxy: those completing graduate studies show half the diabetic rates of those who did not complete high school."} />
          </div>

          <div className="space-y-4 max-w-4xl">
            <p className="text-slate-300 leading-relaxed text-base">
              Health outcomes track directly along financial and educational lines. When observing the income slope, individuals in households earning <strong>Under $10k</strong> experience diabetes rates approaching 40%. This rate drops substantially as we cross the median income threshold (<strong>$35k - $50k</strong>), eventually bottoming out near 15% for those earning <strong>$75k or More</strong>. Similarly, individuals with <strong>No School</strong> or only elementary education face peak diabetic prevalence, whereas those who are <strong>College Graduates</strong> experience drastically lower rates. This pattern illustrates how socioeconomic factors act as a systemic protective mechanism, improving access to preventative medicine and nutrient-dense foods. 
            </p>
            <div>
              <button 
          ...
          <div id="butterfly" className="relative bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl w-full scroll-mt-24">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 pr-12 lg:pr-32">
              <div>
                ...
                <p className="text-sm text-slate-400">Comparing prevalence rates between cohorts. Toggle categories below.</p>
              </div>
              <div className="flex bg-slate-950 p-1.5 rounded-xl border border-slate-800 shadow-inner">
                <button onClick={() => setButterflyView('diet')} className={`px-4 py-2 text-sm rounded-lg font-bold transition-all ${butterflyView === 'diet' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>Diet & Exercise</button>
                <button onClick={() => setButterflyView('barriers')} className={`px-4 py-2 text-sm rounded-lg font-bold transition-all ${butterflyView === 'barriers' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>Systemic Barriers</button>
              </div>
            </div>
            ...
            <div className="w-full h-[350px]">
              <Plot
                data={[
                  { 
                    type: 'bar', x: processedData.butterfly[butterflyView].healthy, y: processedData.butterfly[butterflyView].labels, orientation: 'h', name: 'Healthy Base', marker: { color: '#10b981' },
                    hovertemplate: '<b>%{y}</b><br>Healthy Rate: %{customdata:.1f}%<extra></extra>', customdata: processedData.butterfly[butterflyView].healthy.map(v => Math.abs(v))
                  },
                  { 
                    type: 'bar', x: processedData.butterfly[butterflyView].diabetic, y: processedData.butterfly[butterflyView].labels, orientation: 'h', name: 'Diabetic Base', marker: { color: '#f43f5e' },
                    hovertemplate: '<b>%{y}</b><br>Diabetic Rate: %{x:.1f}%<br><b style="color:#fbbf24">Relative Difference: %{customdata:+.1f}%</b><extra></extra>',
                    customdata: processedData.butterfly[butterflyView].relDiffs
                  }
                ]}
                layout={{ 
                  font: { color: '#94a3b8' }, paper_bgcolor: 'transparent', plot_bgcolor: 'transparent', margin: { l: 150, r: 20, t: 30, b: 40 }, barmode: 'relative', bargap: 0.35, 
                  xaxis: { title: 'Prevalence Rate (%)', tickformat: (val) => Math.abs(val) + '%', range: [-100, 100] }, 
                  yaxis: { gridcolor: 'transparent' },
                  shapes: [{ type: 'line', x0: 0, x1: 0, y0: -0.5, y1: 2.5, line: { color: '#ffffff', width: 4 } }],
                  autosize: true, legend: { orientation: 'h', x: 0.5, xanchor: 'center', y: 1.15 }
                }}
                useResizeHandler={true} style={{ width: '100%', height: '100%' }}
              />
            </div>
            <TakeawayBanner text={butterflyView === 'diet' ? "Hover over the red bars: While dietary intakes only drop slightly, physical activity plummets. Diabetics are relatively ~22% less likely to be physically active." : "Structural health disparities are alarming: patients with diabetes report experiencing difficulty walking over 200% more frequently than the healthy baseline."} />
          </div>
        ...
        <section id="part3" className="space-y-8 scroll-mt-20">
          <div className="border-b border-slate-800 pb-4 mt-16 mb-8">
            <h2 className="text-2xl font-bold text-white flex items-center gap-3"><Stethoscope className="text-blue-500" size={24} /> Intersecting Conditions & Subjective Wellness</h2>
          ...
          <div id="sankey" className="relative bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl w-full space-y-6 scroll-mt-24">
            ...
            <div className="pr-12 lg:pr-32">
              ...
              <p className="text-sm text-slate-400 mt-1">Visualizing how diabetes maps onto secondary cardiovascular complications. Use the colored selection cards below the diagram to view detailed clinical profiles.</p>
            </div>
            <div className="w-full h-[300px]">
              <Plot
                data={[{ type: 'sankey', orientation: 'h', node: { pad: 20, thickness: 30, line: { color: 'transparent', width: 0 }, label: processedData.sankey.nodes, color: ['#f43f5e', '#38bdf8', '#fbbf24', '#a855f7', '#10b981'] }, link: { source: processedData.sankey.links.source, target: processedData.sankey.links.target, value: processedData.sankey.links.value, color: 'rgba(148, 163, 184, 0.15)' } }]}
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 pt-2">
              {[
                ...
                <button 
                  key={card.i} onClick={() => setSelectedNode(card.i)}
          ...
          <div id="violins" className="relative bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl w-full scroll-mt-24">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-6 pr-12 lg:pr-32">
              <div>
                <h4 className="text-lg font-bold text-white mb-1">Continuous BMI Distributions vs. Self-Reported General Health</h4>
                <p className="text-xs text-slate-400">Observe the full density of weights across general health levels (1 = Excellent, 5 = Poor).</p>
              </div>
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 w-full lg:w-72">
                ...
                <input type="range" min="0" max="30" value={mentalHealthDays} onChange={(e) => setMentalHealthDays(parseInt(e.target.value))} className="w-full accent-indigo-500 cursor-pointer" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
              <div className="lg:col-span-4 bg-slate-950/60 p-4 border border-slate-800 rounded-xl space-y-4 text-xs">
                <div>
                  <span className="font-extrabold text-slate-400 uppercase block mb-1">What is a Violin Plot?</span>
              ...
              <div className="lg:col-span-8 h-[380px]">
                <Plot
                  ...
                  layout={{
                    ...
                    showlegend: false, autosize: true, margin: { l: 50, r: 20, t: 15, b: 50 }
                  }}
                  useResizeHandler={true} style={{ width: '100%', height: '100%' }}
                />
              </div>
